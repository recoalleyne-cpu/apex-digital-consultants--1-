...
async function sendEmailWithRetry(payload: EmailPayload): Promise<boolean> {
  ...
  try {
    ...
    console.warn(
      'SMTP verify failed or timed out, proceeding to send attempts',
  ...
  const safeReplyTo = EMAIL_PATTERN.test(String(rawEmail).trim())
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    const firstName =
      clip(parseText(payload.firstName) || parseText(payload.first_name), 120) || splitFirstName;
    const lastName =
    ...
    const companyName = clip(
      parseText(payload.companyName) || parseText(payload.company_name),
      160
    );
    ...
    const id = await createLead(sql, {
      ...
      company: companyName,
      message:
        [serviceInterest, projectDetails, budgetRange].filter(Boolean).join(' | ') || null,
      source
    });
    ...
    return res.status(500).json({
      success: false,
      error: `Lead submission failed: ${message}`
    });
  }
}
import type { VercelRequest, VercelResponse } from '@vercel/node';
...
>>>>>>> origin/feat/email-retry-sanitization
    }
